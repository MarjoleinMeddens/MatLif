classdef lif < handle
    ...
    %
    %  CHANGES:
    %  16-03-23, MM: permute output frames so they match on screen view during
    %  acquisition
    ...
    methods
        ...
        function [sequence, metadata] = loadSeries(obj,seriesNum)
            ...
            % check index
            if seriesNum>obj.NumSeries
                error('lif:SeriesIdxTooHigh','Series %i does not exist, only %i series in lif file',seriesNum,obj.NumSeries);
            end
            ...
            % extract leica metadata (2 versions)
            if strcmp(metadata.leicaMetaData(1:6),'{Image') % new version
                metadata.frameTime = str2double(leicaMetaData.get('Image|ATLConfocalSettingDefinition|FrameTime')); % sec
                metadata.objectiveName = leicaMetaData.get('Image|ATLConfocalSettingDefinition|ObjectiveName');
                metadata.pixelDwellTime = str2double(leicaMetaData.get('Image|ATLConfocalSettingDefinition|PixelDwellTime')); % sec
            else % old version
            ...
            sequence = zeros(obj.SeriesSize(seriesNum,[2 1 3 4 5]),metadata.dataType);
            % retrieve all planes
